...
- e2e 断言了 Claude/Codex 收到的**精确 argv**、标题进程不带 bypass 旗标、worktree 序号（Claude=1、Codex=2、手动开启开关后的 cmd=3；PowerShell 是 ordinary）、会话类型开关的增删条目与重启存活、opt-in agent 默认收起且关闭（展开后 5 个开关全 off、worktree 开关 disabled，开启 Gemini 后菜单出现两个条目，即使已开启重开时仍是收起的）、重启持久化、脏 worktree 删除保护、项目菜单的精确条目（fixture 仓库带一个 GitHub 形态的 `origin`，所以是 5 项且仓库图标为 mono glyph）等——改这些行为必须同步改 `e2e/codefly.spec.ts`。
